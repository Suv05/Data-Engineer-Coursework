...
df_clean = df_clean.withColumn("DeliveryDate", 
    when(col("DeliveryDate").rlike(r'^\d{4}-\d{2}-\d{2}
...
print("\nSpark session stopped. Practice completed!")
         ...
         to_date(col("DeliveryDate"), "yyyy-MM-dd"))
    .otherwise(None))

# Convert string boolean to actual boolean
df_clean = df_clean.withColumn("PromotionApplied", 
    when(col("PromotionApplied") == "TRUE", True)
    .when(col("PromotionApplied") == "FALSE", False)
    .otherwise(None))
