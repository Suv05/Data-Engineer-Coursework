...
if not wait_for_postgres(host="PostgreSQL"):
    exit(1)
...
source_config = {
    'dbname': 'postgres',            # <-- fix this too, your DB is named 'postgres' not 'source_db'
    'user': 'postgres',
    'password': 'postgres',          # <-- FIXED: match docker-compose
    'host': 'PostgreSQL'
}

# Configuration for the destination PostgreSQL database
destination_config = {
    'dbname': 'postgres',            # <-- fix this too, your DB is named 'postgres' not 'destination_db'
    'user': 'postgres',
    'password': 'postgres',          # <-- FIXED: match docker-compose
    'host': 'destination_PostgreSQL'
}
